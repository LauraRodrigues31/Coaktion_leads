...
for (const layout of ["atual", "proposta", "centro"]) {
  const ctx = await browser.newContext({ viewport: { width: 1080, height: 1920 } });
  const page = await ctx.newPage();
  page.setDefaultTimeout(1500);
  await page.goto(`http://localhost:4173/?totem=1${layout === "atual" ? "" : "&layout=" + layout}`);
  await page.waitForSelector("text=Toque na tela");
  await page.screenshot({ path: `${OUT}/${layout}-00-welcome.png` });
  ...
  for (let i = 0; i < 40; i++) {
    ...
    try {
      ...
      if (await page.getByText("Obrigado por fazer seu pedido").count()) { await page.screenshot({ path: `${OUT}/${layout}-99-ticket.png` }); break; }
